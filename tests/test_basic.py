...
def test_zonal_mean():
    ...
    assert da_zonal_mean.dims == ("plev", "lat")
...
def test_meridional_mean():
    ...
    assert da_meridional_mean.dims == ("plev", "lon")
...
def test_meridional_mean_wrong_lat_order_fail():
    ...
    with pytest.raises(ValueError) as e:
        _ = meridional_mean(da, 90, -90)
    assert "lat1 must be less than lat2" in str(e.value)


def test_meridional_mean_strict_too_few_lats_fail():
    """Test that meridional mean fails with too few lats in strict mode"""
    da = create_dummy_geo_field(lon_coord(10), lat_coord(10), plev_coord(3))
    with pytest.raises(ValueError) as e:
        _ = meridional_mean(da.sel(lat=slice(30, 60)), 0, 90, strict=True)
